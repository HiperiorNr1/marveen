...
export function saveAgentMemory(
  ...
  // Fire-and-forget: generate embedding asynchronously. A null result means the
  // embed failed; escalate to a rate-limited WARN only if embeddings worked
  // earlier in this process and then started failing (a real Ollama outage --
  // see shouldWarnEmbedFailure). Otherwise generateEmbedding's own DEBUG log is
  // enough (Ollama simply never installed). The memory is still saved either
  // way; POST /api/memories/reembed backfills the missing embedding.
  generateEmbedding(content + (keywords ? ' ' + keywords : '')).then(emb => {
    ...
    } else if (shouldWarnEmbedFailure(hadEmbeddingSuccess, Date.now(), lastEmbedFailWarnAt, EMBED_FAIL_WARN_INTERVAL_MS)) {
      lastEmbedFailWarnAt = Date.now()
      logger.warn({ agentId, id }, 'Memory embedding failed after prior success -- Ollama outage? Memory saved without embedding; POST /api/memories/reembed to backfill')
    }
  }).catch(() => {})
...
// Embedding outage visibility (card 6d7d14a8). The fire-and-forget memory embed
// silently swallowed nulls, so when Ollama died (2026-06-03) 181 memories
// accumulated NULL embeddings unnoticed for days. We escalate to WARN ONLY once
// embeddings have actually worked in this process and then start failing (a real
// outage) -- never on a machine where Ollama was simply never installed (the
// common upstream case, where generateEmbedding already DEBUG-logs).
let hadEmbeddingSuccess = false
let lastEmbedFailWarnAt = 0
export const EMBED_FAIL_WARN_INTERVAL_MS = 10 * 60 * 1000

// Pure gate: should a failed background embed escalate to a WARN? True only when
// embeddings previously succeeded (so this is an outage, not Ollama-absent) AND
// the rate-limit window has elapsed since the last WARN. Unit-tested.
export function shouldWarnEmbedFailure(
  hadSuccess: boolean,
  nowMs: number,
  lastWarnMs: number,
  intervalMs: number,
): boolean {
  return hadSuccess && nowMs - lastWarnMs > intervalMs
}
...
export async function generateEmbedding(text: string): Promise<number[] | null> {
  try {
    ...
    if (emb) hadEmbeddingSuccess = true
    return emb
  } catch (err) {
