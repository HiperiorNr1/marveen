...
/** True if `cmd` looks like a channel-plugin poller for the given provider.
 * Pure string match -- safe to unit-test against captured ps lines. */
export function matchesProviderPollerCmd(cmd: string, provider: ChannelProviderType): boolean {
  if (provider === 'telegram') {
    // Telegram plugin spawns `bun ... /<plugin-path>/telegram/server.ts ...`
    // under the claude pid. The two clauses match the historical paths;
  ...
  if (provider === 'discord') {
    return cmd.includes('discord') && (cmd.includes('node') || cmd.includes('bun'))
  }
  // slack
